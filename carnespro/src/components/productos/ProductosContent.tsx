...
import {
  Beef, Search, Plus, Tag, AlertTriangle,
...
import { MOCK_STOCK_ALERTS } from "@/lib/constants";
...
export default function ProductosContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (modal === "new") {
      addProduct({ plu: form.plu || `${Date.now()}`.slice(-5), name: form.name, category: form.category, price, unit: form.unit, emoji: form.emoji });
    } else if (modal === "edit" && editingId) {
      updateProduct(editingId, { plu: form.plu, name: form.name, category: form.category, price, unit: form.unit, emoji: form.emoji });
    }
    closeModal();
  };
    ...
    <div className="page-container" style={{ animation: "fadeInUp 0.4s ease-out" }}>
      ...
      <div className="stats-grid animate-in">
        ...
        <div className="stat-card stat-card--ticket">
          <div className="stat-card__top">
            <div className="stat-card__icon stat-card__icon--ticket"><AlertTriangle size={20} /></div>
          </div>
          <div className="stat-card__value">{MOCK_STOCK_ALERTS.length}</div>
